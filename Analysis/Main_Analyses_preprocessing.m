...
if any(ismember(fieldnames(subjectdata),'subjectses'))
    fprintf('SESSION: %s\n',subjectdata.subjectses)
end
...
if preprodat < length(curexperiment.datasets_names)
    ...
    if artfindat ~= length(curexperiment.datasets)
        % run a script that rejects the non-ocular artifacts
        fprintf('\nFINAL ARTIFACT REJECTION\n');
        eval([curexperiment.name '_ArtifactRejectionFinal'])
    else
       fprintf('\nLOADING FINAL ARTIFACT REJECTED DATA\n');
        for d=1:length(curexperiment.datasets)
            load(fullfile(subjectdata.subjectdir, strcat(subjectdata.subjectnr,curexperiment.dataset_name{d},'_ArtiRemovedFin.mat')));
        end
        evalc(curexperiment.define_datasets);
    end            
    clear artfindat 

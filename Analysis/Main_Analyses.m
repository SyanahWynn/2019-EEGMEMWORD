%% MAIN ANALYSES
% FamRecEEG made with fieldtrip-20160401
...
for f=1:file_n
    ...
    if preprodat >= preproc_check
        ...
        if ana
            ...
            if any(strcmp('erp',curexperiment.analyses))
                %% ERP ANALYSES ON SUBJECT LEVEL
                %run a script that does time-frequency analyses on subject level
                if erp ~= curexperiment.Nanalyses.erp || ep ~= curexperiment.Nanalyses.ep
                    fprintf('\nERP ANALYSES\n');
                    eval([curexperiment.name '_ERP_Analyses'])
                end
            end
            clear erp e
            ...
            if any(strcmp('pow',curexperiment.analyses))
                ...
                %run a script that does time-frequency analyses on subject level
                if tp ~= curexperiment.Nanalyses.tp %|| ip ~= curexperiment.Nanalyses.ip
...
if any(strcmp(curexperiment.analyses,'erp'))
    %% ERP ANALYSES GRAND AVERAGE
    % run a script that does ERP analyses across subjects
    eval([curexperiment.name '_ERP_Analyses_GrandAverage'])
end
